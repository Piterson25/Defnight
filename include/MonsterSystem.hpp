...
class MonsterSystem {
public:
    MonsterSystem(const sf::VideoMode &vm, TileMap *tileMap, const float &gridSize,
    ...
    void addMonster(const std::string &monster_name, sf::Texture &texture, const float &x,
                    const float &y, const float &wave_mod);
    void addBoss(const std::string &monster_name, sf::Texture &texture, const float &x,
                 const float &y, const float &wave_mod);
    ...
    void update(Player *player, PlayerGUI *playerGUI, ProjectileSystem *projectileSystem,
