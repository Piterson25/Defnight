...
{
	this->floatingTexts.emplace_back(new FloatingText(this->font, text, charSize, posX, posY, color, isgui, this->vm));
}
...
void FloatingTextSystem::update(const float& dt)
{
	...
	for (auto text = this->floatingTexts.begin(); text != this->floatingTexts.end();) {
		if ((*text)->isOver()) text = this->floatingTexts.erase(text);
		else ++text;
	}
}
