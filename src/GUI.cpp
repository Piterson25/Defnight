...
gui::ButtonText::ButtonText(sf::Font *font, const std::string &text, const unsigned &charSize,
                            const float &posX, const float &posY, const sf::Color &idleColor,
                            const sf::Color &hoverColor, const bool &center)
{
...
void gui::ButtonText::center(const float &posX)
{
    this->text.setPosition(
        static_cast<float>(static_cast<int>(posX - (this->text.getGlobalBounds().width) / 2)),
...
gui::ButtonSprite::ButtonSprite(const std::string &texturePath, const float &posX,
                                const float &posY, const float &scale, const bool &center)
{
    this->buttonState = BUTTON_IDLE;
...
gui::ButtonSprite::ButtonSprite(const sf::Texture &texture, const float &posX, const float &posY,
                                const float &scale, const bool &center)
{
    this->buttonState = BUTTON_IDLE;
...
void gui::ButtonSprite::center(const float &posX)
{
    this->sprite.setPosition(
        static_cast<float>(static_cast<int>(posX - (this->sprite.getGlobalBounds().width) / 2)),
...
gui::Sprite::Sprite(const std::string &texturePath, const float &posX, const float &posY,
                    const float &scale, const bool &center)
{
    this->texture.loadFromFile(texturePath);
    this->sprite.setTexture(this->texture);
...
gui::Sprite::Sprite(const sf::Texture &texture, const float &posX, const float &posY,
                    const float &scale, const bool &center)
{
    this->texture = texture;
    this->sprite.setTexture(this->texture);
...
gui::Sprite::Sprite(sf::Sprite &sprite, const float &posX, const float &posY, const float &scale,
                    const bool &center)
    : sprite(sprite)
{
