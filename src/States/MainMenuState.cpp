...
MainMenuState::MainMenuState(float gridSize, sf::RenderWindow &window, GameSettings &gameSettings,
...
void MainMenuState::initGUI()
{
    ...
    this->dimBackground.setSize(sf::Vector2f(static_cast<float>(vm.size.x), static_cast<float>(vm.size.y)));
    ...
    this->dimAlpha = 0.f;
    this->sprites["LOGO"] = std::make_unique<gui::Sprite>("assets/textures/icon.png", calcX(640, vm), calcY(232, vm),
                                                          calcScale(8, vm), true);
    ...
    this->dimMap.setSize(sf::Vector2f(static_cast<float>(vm.size.x), static_cast<float>(vm.size.y)));
    this->dimMap.setFillColor(sf::Color(0, 0, 0, 192));

    this->quitBackground.setSize(sf::Vector2f(static_cast<float>(vm.size.x), static_cast<float>(vm.size.y)));
    this->quitBackground.setFillColor(sf::Color(0, 0, 0, 192));

    this->sprites["TITLE"] = std::make_unique<gui::Sprite>("assets/textures/title.png", calcX(640, vm), calcY(144, vm),
                                                           calcScale(1, vm), true);

    this->text_buttons["PLAY"] = std::make_unique<gui::ButtonText>(this->lang["PLAY"], calcChar(24, vm), calcX(640, vm),
                                                                   calcY(300, vm), gui::WHITE, gui::LIGHT_GREY, true);
    this->text_buttons["STATISTICS"] = std::make_unique<gui::ButtonText>(
        this->lang["STATISTICS"], calcChar(24, vm), calcX(640, vm), calcY(372, vm), gui::WHITE, gui::LIGHT_GREY, true);
    this->text_buttons["SETTINGS"] = std::make_unique<gui::ButtonText>(
        this->lang["SETTINGS"], calcChar(24, vm), calcX(640, vm), calcY(444, vm), gui::WHITE, gui::LIGHT_GREY, true);
    this->text_buttons["CREDITS"] = std::make_unique<gui::ButtonText>(
        this->lang["CREDITS"], calcChar(24, vm), calcX(640, vm), calcY(516, vm), gui::WHITE, gui::LIGHT_GREY, true);
    this->text_buttons["QUIT"] = std::make_unique<gui::ButtonText>(this->lang["QUIT"], calcChar(24, vm), calcX(640, vm),
                                                                   calcY(588, vm), gui::WHITE, gui::LIGHT_GREY, true);

    this->texts["TOTAL_XP"] =
        std::make_unique<gui::Text>(this->lang["TOTAL_XP"] + std::to_string(playerData.xp), calcChar(16, vm),
                                    calcX(8, vm), calcY(680, vm), gui::WHITE, false);

    this->text_buttons["RANK"] = std::make_unique<gui::ButtonText>(this->lang["RANK"], calcChar(16, vm), calcX(8, vm),
                                                                   calcY(700, vm), gui::WHITE, gui::LIGHT_GREY, false);

    this->texts["PLAYER_RANK"] = std::make_unique<gui::Text>(this->lang["RANK"], calcChar(16, vm),
                                                             this->text_buttons["RANK"]->getPosition().x +
                                                                 this->text_buttons["RANK"]->getWidth(),
                                                             calcY(700, vm), gui::WHITE, false);

    this->texts["VERSION"] =
        std::make_unique<gui::Text>(VERSION, calcChar(16, vm), calcX(1272, vm), calcY(700, vm), gui::WHITE, false);
    this->texts["VERSION"]->setPosition(
        sf::Vector2f(calcX(1272, vm) - this->texts["VERSION"]->getWidth(), calcY(700, vm)));

    this->quitwindow = false;

    this->texts["ARE_YOU_SURE"] = std::make_unique<gui::Text>(this->lang["ARE_YOU_SURE"], calcChar(32, vm),
                                                              calcX(640, vm), calcY(250, vm), gui::WHITE, true);
    this->text_buttons["YES"] = std::make_unique<gui::ButtonText>(this->lang["YES"], calcChar(32, vm), calcX(488, vm),
                                                                  calcY(306, vm), gui::WHITE, gui::LIGHT_GREY, false);
    this->text_buttons["NO"] = std::make_unique<gui::ButtonText>(this->lang["NO"], calcChar(32, vm), calcX(704, vm),
                                                                 calcY(306, vm), gui::WHITE, gui::LIGHT_GREY, false);
    ...
    this->sprite_buttons["HERO1"] = std::make_unique<gui::ButtonSprite>(gui::RECT_BUTTON, calcX(64, vm), calcY(256, vm),
                                                                        calcScale(2, vm), gui::GREY, gui::WHITE, false);
    this->sprites["HERO1"] = std::make_unique<gui::Sprite>("assets/textures/upgrades_icons.png", calcX(88, vm),
                                                           calcY(280, vm), calcScale(8, vm), false);
    this->sprites["HERO1"]->setTextureRect(sf::IntRect({0, 0}, {16, 16}));
    this->texts["WARRIOR"] = std::make_unique<gui::Text>(this->lang["WARRIOR"], calcChar(32, vm), calcX(150, vm),
                                                         calcY(200, vm), gui::WHITE, true);

    this->choosing_hero = false;

    this->text_buttons["CHOOSE"] = std::make_unique<gui::ButtonText>(
        this->lang["CHOOSE"], calcChar(32, vm), calcX(240, vm), calcY(564, vm), gui::WHITE, gui::LIGHT_GREY, true);
    this->sprites["HERO_PREVIEW"] = std::make_unique<gui::Sprite>("assets/textures/upgrades_icons.png", calcX(640, vm),
    ...
    this->sprites["HP_FRAME"] = std::make_unique<gui::Sprite>(framesTexture, calcX(860, vm), calcY(498, vm),
                                                              calcScale(1, vm), true, sf::IntRect({88, 0}, {272, 36}));
    this->sprites["HP_BAR"] = std::make_unique<gui::Sprite>("assets/textures/bars.png", calcX(860, vm), calcY(506, vm),
                                                            calcScale(1, vm), true);
    this->sprites["HP_BAR"]->setTextureRect(sf::IntRect({0, 20}, {256, 20}));
    this->sprites["HP_BAR"]->center(calcX(860, vm));
    this->texts["HP"] =
        std::make_unique<gui::Text>("HP:10/10", calcChar(16, vm), calcX(860, vm), calcY(509, vm), gui::WHITE, true);
    this->sprites["SPRINT_FRAME"] = std::make_unique<gui::Sprite>(
        framesTexture, calcX(860, vm), calcY(538, vm), calcScale(1, vm), true, sf::IntRect({88, 0}, {272, 36}));
    this->sprites["SPRINT_BAR"] = std::make_unique<gui::Sprite>("assets/textures/bars.png", calcX(860, vm),
                                                                calcY(546, vm), calcScale(1, vm), true);
    this->sprites["SPRINT_BAR"]->setTextureRect(sf::IntRect({0, 40}, {256, 20}));
    this->sprites["SPRINT_BAR"]->center(calcX(860, vm));
    this->texts["SPRINT"] =
        std::make_unique<gui::Text>("100/100", calcChar(16, vm), calcX(860, vm), calcY(549, vm), gui::WHITE, true);

    this->attributes_texture = sf::Texture("assets/textures/attributes_icons.png");
    for (int i = 0; i < 10; ++i) {
    ...
    for (int y = 0; y < 3; y++) {
        for (int x = 0; x < 3; x++) {
            this->abilties.push_back(std::make_unique<gui::Sprite>(this->abilities_texture, calcX(432.f + 48.f * x, vm),
    ...
    this->sprites["ARMOR"] =
        std::make_unique<gui::Sprite>(attribute_vec[1], calcX(728, vm), calcY(586, vm), calcScale(2, vm), false);
    this->texts["ARMOR"] =
        std::make_unique<gui::Text>("3", calcChar(16, vm), calcX(744, vm), calcY(640, vm), gui::LIGHT_GREY, true);
    this->sprites["REG"] =
        std::make_unique<gui::Sprite>(attribute_vec[2], calcX(792, vm), calcY(586, vm), calcScale(2, vm), false);
    this->texts["REG"] =
        std::make_unique<gui::Text>("1", calcChar(16, vm), calcX(808, vm), calcY(640, vm), gui::RED, true);
    this->sprites["ATTACK"] =
        std::make_unique<gui::Sprite>(attribute_vec[5], calcX(856, vm), calcY(586, vm), calcScale(2, vm), false);
    this->texts["ATTACK"] =
        std::make_unique<gui::Text>("5", calcChar(16, vm), calcX(872, vm), calcY(640, vm), gui::LIGHT_GREY, true);
    this->sprites["ATTACK_SPEED"] =
        std::make_unique<gui::Sprite>(attribute_vec[6], calcX(920, vm), calcY(586, vm), calcScale(2, vm), false);
    this->texts["ATTACK_SPEED"] =
        std::make_unique<gui::Text>("4", calcChar(16, vm), calcX(936, vm), calcY(640, vm), gui::LIGHT_GREY, true);
    this->sprites["SPEED"] =
        std::make_unique<gui::Sprite>(attribute_vec[7], calcX(984, vm), calcY(586, vm), calcScale(2, vm), false);
    this->texts["SPEED"] =
        std::make_unique<gui::Text>("4", calcChar(16, vm), calcX(1000, vm), calcY(640, vm), gui::LIGHT_GREY, true);
    this->sprites["CRITICAL"] =
        std::make_unique<gui::Sprite>(attribute_vec[8], calcX(1048, vm), calcY(586, vm), calcScale(2, vm), false);
    this->texts["CRITICAL"] =
        std::make_unique<gui::Text>("20%", calcChar(16, vm), calcX(1064, vm), calcY(640, vm), gui::LIGHT_GREY, true);
    ...
    this->sprite_buttons["DIFFICULTY1"] = std::make_unique<gui::ButtonSprite>(
        gui::RECT_DIFFICULTY, calcX(20, vm), calcY(184, vm), calcScale(1, vm), gui::GREY, gui::WHITE, false);
    this->sprite_buttons["DIFFICULTY2"] = std::make_unique<gui::ButtonSprite>(
        gui::RECT_DIFFICULTY, calcX(440, vm), calcY(184, vm), calcScale(1, vm), gui::GREY, gui::WHITE, false);
    this->sprite_buttons["DIFFICULTY3"] = std::make_unique<gui::ButtonSprite>(
        gui::RECT_DIFFICULTY, calcX(860, vm), calcY(184, vm), calcScale(1, vm), gui::GREY, gui::WHITE, false);
    this->sprite_buttons["DIFFICULTY4"] =
        std::make_unique<gui::ButtonSprite>(gui::RECT_DIFFICULTY_EXTREME, calcX(640, vm), calcY(604, vm),
                                            calcScale(1, vm), gui::DARK_RED, gui::WHITE, true);

    this->sprites["DIFFICULTY1"] = std::make_unique<gui::Sprite>("assets/textures/difficulty_icons.png", calcX(168, vm),
                                                                 calcY(224, vm), calcScale(16, vm), false);
    this->sprites["DIFFICULTY1"]->setTextureRect(sf::IntRect({0, 0}, {7, 6}));
    this->sprites["DIFFICULTY2"] = std::make_unique<gui::Sprite>("assets/textures/difficulty_icons.png", calcX(584, vm),
                                                                 calcY(224, vm), calcScale(16, vm), false);
    this->sprites["DIFFICULTY2"]->setTextureRect(sf::IntRect({7, 0}, {7, 6}));
    this->sprites["DIFFICULTY3"] = std::make_unique<gui::Sprite>(
        "assets/textures/difficulty_icons.png", calcX(1000, vm), calcY(224, vm), calcScale(16, vm), false);
    this->sprites["DIFFICULTY3"]->setTextureRect(sf::IntRect({14, 0}, {7, 6}));
    this->sprites["DIFFICULTY3"]->center(calcX(1056, vm));
    this->sprites["DIFFICULTY4_1"] = std::make_unique<gui::Sprite>(
        "assets/textures/difficulty_icons.png", calcX(398, vm), calcY(626, vm), calcScale(4, vm), false);
    this->sprites["DIFFICULTY4_1"]->setTextureRect(sf::IntRect({21, 0}, {13, 12}));
    this->sprites["DIFFICULTY4_2"] = std::make_unique<gui::Sprite>(
        "assets/textures/difficulty_icons.png", calcX(830, vm), calcY(626, vm), calcScale(4, vm), false);
    this->sprites["DIFFICULTY4_2"]->setTextureRect(sf::IntRect({21, 0}, {13, 12}));

    this->texts["EASY"] = std::make_unique<gui::Text>(this->lang["EASY"], calcChar(32, vm), calcX(224, vm),
                                                      calcY(364, vm), gui::WHITE, true);
    this->texts["NORMAL"] = std::make_unique<gui::Text>(this->lang["NORMAL"], calcChar(32, vm), calcX(640, vm),
                                                        calcY(364, vm), gui::WHITE, true);
    this->texts["HARD"] = std::make_unique<gui::Text>(this->lang["HARD"], calcChar(32, vm), calcX(1056, vm),
                                                      calcY(364, vm), gui::WHITE, true);
    this->texts["EXTREME"] = std::make_unique<gui::Text>(this->lang["EXTREME"], calcChar(32, vm), calcX(640, vm),
                                                         calcY(638, vm), gui::WHITE, true);
    this->texts["EASY_DESC"] =
        std::make_unique<gui::Text>(this->lang["MONSTERS_HAVE"] + "\n\n-25% HP\n\n-25% " + this->lang["ATTACK"] +
                                        "\n\n50% " + this->lang["HEART_DROP"],
                                    calcChar(16, vm), calcX(224, vm), calcY(426, vm), gui::RED, true);
    this->texts["NORMAL_DESC"] = std::make_unique<gui::Text>(this->lang["NORMAL_DESC"], calcChar(16, vm),
                                                             calcX(640, vm), calcY(426, vm), gui::RED, true);
    this->texts["HARD_DESC"] = std::make_unique<gui::Text>(
        this->lang["MONSTERS_HAVE"] + "\n\n+25% HP\n\n+25% " + this->lang["ATTACK"] + "\n\n" + this->lang["TIME_DROP"],
        calcChar(16, vm), calcX(1056, vm), calcY(426, vm), gui::RED, true);
    this->texts["EXTREME_DESC_1"] =
        std::make_unique<gui::Text>("? ? ?", calcChar(32, vm), calcX(304, vm), calcY(638, vm), gui::RED, true);
    this->texts["EXTREME_DESC_2"] =
        std::make_unique<gui::Text>("? ? ?", calcChar(32, vm), calcX(976, vm), calcY(638, vm), gui::RED, true);
    ...
    this->texts["RANKS"] = std::make_unique<gui::Text>(this->lang["RANKS"], calcChar(32, vm), calcX(640, vm),
                                                       calcY(96, vm), gui::WHITE, true);

    this->sprites["LEFT_ARROW"] = std::make_unique<gui::Sprite>("assets/textures/selects.png", calcX(192, vm),
                                                                calcY(192, vm), calcScale(2, vm), false);
    this->sprites["LEFT_ARROW"]->setTextureRect(gui::RECT_SMALL_ARROW);
    this->sprites["LEFT_ARROW"]->flipHorizontal();
    this->sprites["RIGHT_ARROW"] = std::make_unique<gui::Sprite>("assets/textures/selects.png", calcX(592, vm),
                                                                 calcY(192, vm), calcScale(2, vm), false);
    ...
    for (int i = PlayerStats::ranks.size() - 1; i >= 0; i--) {
        const std::string xpRange = std::to_string(PlayerStats::ranks[i].minXP) + "-" +
                                    (i > 0 ? std::to_string(PlayerStats::ranks[i - 1].minXP - 1) : "0") + " XP";

        this->ranksTexts.push_back(RankText{
            std::make_unique<gui::Text>(this->lang[PlayerStats::ranks[i].name], calcChar(16, vm), calcX(192, vm),
                                        calcY(positionY, vm), PlayerStats::ranks[i].color, false),
            std::make_unique<gui::Text>(xpRange, calcChar(16, vm), calcX(384, vm), calcY(positionY, vm), gui::WHITE,
                                        false)});

        if (i == 0) {
            this->ranksTexts.back().xp->setText(std::to_string(PlayerStats::ranks[i].minXP) + "+ XP");
        }

        positionY += 48;
    }

    this->texts["PLAYER_XP"] =
        std::make_unique<gui::Text>(this->lang["TOTAL_XP"] + std::to_string(playerData.xp), calcChar(24, vm),
                                    calcX(960, vm), calcY(192, vm), gui::WHITE, true);

    this->texts["CURRENT_RANK"] = std::make_unique<gui::Text>(this->lang["CURRENT_RANK"], calcChar(16, vm),
                                                              calcX(960, vm), calcY(240, vm), gui::WHITE, true);

    this->texts["CURRENT_PLAYER_RANK"] = std::make_unique<gui::Text>(this->lang["NOVICE"], calcChar(16, vm),
    ...
    this->texts["CURRENT_RANK"]->setPositionX(
        calcX(960, vm) -
        (this->texts["CURRENT_RANK"]->getWidth() + this->texts["CURRENT_PLAYER_RANK"]->getWidth()) / 2);

    this->texts["CURRENT_PLAYER_RANK"]->setPositionX(this->texts["CURRENT_RANK"]->getPosition().x +
                                                     this->texts["CURRENT_RANK"]->getWidth());

    this->texts["NEXT_RANK"] = std::make_unique<gui::Text>(this->lang["NEXT_RANK"], calcChar(16, vm), calcX(960, vm),
                                                           calcY(288, vm), gui::WHITE, true);

    this->texts["NEXT_PLAYER_RANK"] =
        std::make_unique<gui::Text>(this->lang["RECRUIT"], calcChar(16, vm),
                                    this->texts["NEXT_RANK"]->getPosition().x + this->texts["NEXT_RANK"]->getWidth(),
                                    calcY(288, vm), gui::LIGHT_GREY, false);

    this->texts["NEXT_RANK"]->setPositionX(
        calcX(960, vm) - (this->texts["NEXT_RANK"]->getWidth() + this->texts["NEXT_PLAYER_RANK"]->getWidth()) / 2);

    this->texts["NEXT_PLAYER_RANK"]->setPositionX(this->texts["NEXT_RANK"]->getPosition().x +
                                                  this->texts["NEXT_RANK"]->getWidth());

    this->sprites["XP_BAR"] = std::make_unique<gui::Sprite>("assets/textures/bars.png", calcX(832, vm), calcY(344, vm),
                                                            calcScale(1, vm), false);
    this->sprites["XP_BAR"]->setTextureRect(sf::IntRect({0, 0}, {0, 0}));
    this->sprites["XP_FRAME"] = std::make_unique<gui::Sprite>(framesTexture, calcX(960, vm), calcY(336, vm),
                                                              calcScale(1, vm), true, sf::IntRect({88, 0}, {272, 36}));
    this->texts["PERCENT"] =
        std::make_unique<gui::Text>("0%", calcChar(16, vm), calcX(960, vm), calcY(347, vm), gui::WHITE, true);

    this->texts["CURRENT_RANK_XP"] =
        std::make_unique<gui::Text>("0", calcChar(16, vm), calcX(824, vm), calcY(384, vm), gui::WHITE, true);

    this->texts["NEXT_RANK_XP"] =
        std::make_unique<gui::Text>("99", calcChar(16, vm), calcX(1096, vm), calcY(384, vm), gui::WHITE, true);
}
...
void MainMenuState::update(float dt)
{
    ...
    if (!this->loadedPlayerData) {
        ...
        this->texts["TOTAL_XP"]->setText(this->lang["TOTAL_XP"] + std::to_string(playerData.xp));

        this->texts["PLAYER_XP"]->setText(this->lang["TOTAL_XP"] + std::to_string(playerData.xp));
        this->texts["PLAYER_XP"]->center(calcX(960, vm));
        this->setPlayerRank();
    }

    if (this->page >= 1) {
        if (!this->map.getGlobalBounds().findIntersection(this->mapView.getViewport())) {
            this->mapVelocity = sf::Vector2f(32.f * cos((3.1415f / 180.f)), 32.f * cos((3.1415f / 180.f)));
            this->mapRotate = Random::Float() * 360.f - 90.f;
            this->map.setPosition({256.f, 256.f});
            this->map.setRotation(sf::degrees(this->mapRotate));
        ...
        switch (this->page) {
            case 0:
                ...
                if (this->introCooldown > 1.f) {
                    if (this->introCooldown > 5.f) {
                        ...
                        this->sprites["LOGO"]->setColor(
                            sf::Color(255, 255, 255, static_cast<uint8_t>(this->dimAlpha * 255.f)));
                    }
                }
                break;
            case 1:
                if (!quitwindow) {
                    if (this->text_buttons["PLAY"]->isPressed(this->mousePosWindow)) {
                        ...
                        this->soundEngine.addSound("button");
                        this->states.push(new StatsState(this->gridSize, this->window, this->gameSettings,
                                                         this->soundEngine, this->musicEngine, this->states));
                    }
                    else if (this->text_buttons["SETTINGS"]->isPressed(this->mousePosWindow)) {
                        this->soundEngine.addSound("button");
                        this->states.push(new SettingsState(this->gridSize, this->window, this->gameSettings,
                                                            this->soundEngine, this->musicEngine, this->states));
                    }

                    else if (this->text_buttons["CREDITS"]->isPressed(this->mousePosWindow)) {
                        this->soundEngine.addSound("button");
                        this->page = 5;
                    }
                    else if (this->text_buttons["QUIT"]->isPressed(this->mousePosWindow)) {
                        this->soundEngine.addSound("button");
                        this->quitwindow = true;
                    }
                    else if (this->text_buttons["RANK"]->isPressed(this->mousePosWindow)) {
                        this->soundEngine.addSound("button");
                        this->page = 6;
                    }
                }
                else {
                    if (this->text_buttons["YES"]->isPressed(this->mousePosWindow)) {
                        this->soundEngine.addSound("button");
                        this->endState();
                    }
                    else if (this->text_buttons["NO"]->isPressed(this->mousePosWindow)) {
                        this->soundEngine.addSound("button");
                        this->quitwindow = false;
                    }
                }
            ...
            case 3:
                ...
                if (this->choosing_hero) {
                    if (this->text_buttons["CHOOSE"]->isPressed(this->mousePosWindow)) {
                        this->soundEngine.addSound("button");
                        this->page = 4;
                    }
                }
...
void MainMenuState::draw(sf::RenderTarget *target)
{
    ...
    switch (this->page) {
        ...
        case 4:
            ...
            this->sprites["DIFFICULTY4_1"]->draw(*target);
            this->sprites["DIFFICULTY4_2"]->draw(*target);
...
void MainMenuState::fadingEffect(float dt)
{
    ...
    this->dimBackground.setFillColor(sf::Color(0, 0, 0, static_cast<uint8_t>(this->dimAlpha * 255.f)));
}
...
void MainMenuState::setPlayerRank()
{
    ...
    for (auto it = PlayerStats::ranks.begin(); it != PlayerStats::ranks.end(); ++it) {
        if (this->playerData.xp >= it->minXP) {
            if (it != PlayerStats::ranks.begin()) {
                const float percent = ((playerData.xp - it->minXP) * 100.f / (std::prev(it)->minXP - it->minXP));

                this->texts["PERCENT"]->setText(std::to_string(static_cast<int>(percent)) + "%");
                this->texts["PERCENT"]->center(calcX(960, vm));
                this->sprites["XP_BAR"]->setTextureRect(
                    sf::IntRect({0, 0}, {static_cast<int>(percent * 256.f / 100.f), 20}));
                this->texts["NEXT_RANK_XP"]->setText(std::to_string(std::prev(it)->minXP));
                this->texts["NEXT_RANK_XP"]->center(calcX(1096, vm));
                this->texts["NEXT_PLAYER_RANK"]->setText(this->lang[std::prev(it)->name]);
                this->texts["NEXT_PLAYER_RANK"]->setFillColor(std::prev(it)->color);

                this->texts["NEXT_RANK"]->setPositionX(
                    calcX(960, vm) -
                    (this->texts["NEXT_RANK"]->getWidth() + this->texts["NEXT_PLAYER_RANK"]->getWidth()) / 2);
    ...
    this->texts["CURRENT_PLAYER_RANK"]->setText(this->texts["PLAYER_RANK"]->getText());
    this->texts["CURRENT_PLAYER_RANK"]->setFillColor(this->texts["PLAYER_RANK"]->getFillColor());
    ...
    for (const auto &rank : ranksTexts) {
        if (rank.rank->getText() == this->lang[this->playerRank.name]) {
            this->sprites["LEFT_ARROW"]->setPosition(
                sf::Vector2f(rank.rank->getPosition().x - calcX(32, vm), rank.rank->getPosition().y - calcY(8, vm)));
            this->sprites["RIGHT_ARROW"]->setPosition(
                sf::Vector2f(rank.xp->getPosition().x + rank.xp->getWidth() + calcX(32, vm),
