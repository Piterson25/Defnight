...
GameState::GameState(const float &gridSize, sf::RenderWindow *window, GameSettings *grap,
                     ...
                     std::stack<State *> *states, const std::string &map_name,
                     const std::string &hero_name, const std::string &difficulty_name)
    : State(gridSize, window, grap, supportedKeys, font, soundEngine, musicEngine, states)
{
    ...
    this->playerGUI =
        new PlayerGUI(&this->font, this->player, this->gameSettings->resolution,
                      this->gameSettings->soundsVolume, hero_name, difficulty_name, this->lang);

    this->view.setSize(sf::Vector2f(static_cast<float>(vm.width), static_cast<float>(vm.height)));
    this->view.setCenter(sf::Vector2f(this->player->getPosition().x + calcX(32, vm),
                                      this->player->getPosition().y + calcY(32, vm)));

    this->viewHUD.setSize(
        sf::Vector2f(static_cast<float>(vm.width), static_cast<float>(vm.height)));
    this->viewHUD.setCenter(
        sf::Vector2f(static_cast<float>(vm.width) / 2.f, static_cast<float>(vm.height) / 2.f));
...
void GameState::update(const float &dt)
{
    ...
    if (this->player->isDead()) {
        const uint16_t result =
            this->playerGUI->updateDeathScreenButtons(this->mousePosWindow, this->getMouseClick());
        if (result == 1) {
            this->setMouseClick(true);
    ...
    if (this->playerGUI->getIsEscape() &&
        (!this->playerGUI->getIsLeveling() && !this->playerGUI->getIsUpgrading())) {
        const uint8_t result =
            this->playerGUI->updateEscapeButton(this->mousePosWindow, this->getMouseClick());
        if (result == 1) {
            this->soundEngine->addSound("button");
            this->endState();
        }
            ...
            this->states->push(new SettingsState(
                this->gridSize, this->window, this->gameSettings, this->supportedKeys, &this->font,
    ...
    if (!this->paused) {
        if (this->player->getSpawned()) {
            ...
            if (this->player->hasVelocity()) {
                ...
                const bool osY =
                    this->player->getPosition().y <= calcY(200, this->gameSettings->resolution) ||
                    this->player->getPosition().y >= this->background.getGlobalBounds().height -
                                                         calcY(392, this->gameSettings->resolution);
                const bool osX =
                    this->player->getPosition().x <= calcX(608, this->gameSettings->resolution) ||
                    this->player->getPosition().x >= this->background.getGlobalBounds().width -
                                                         calcX(672, this->gameSettings->resolution);

                if (osX || osY) {
                    if (osX && osY) {
                        this->view.setCenter(
                            sf::Vector2f(this->view.getCenter().x, this->view.getCenter().y));
                    }
                    else {
                        if (osX) {
                            this->view.setCenter(sf::Vector2f(this->view.getCenter().x,
                                                              this->player->getPosition().y + _32));
                        }
                        else if (osY) {
                            this->view.setCenter(sf::Vector2f(this->player->getPosition().x + _32,
                                                              view.getCenter().y));
                        }
                    }
                }
            ...
            if (this->mousePosView.y > calcY(128, this->gameSettings->resolution)) {
                ...
                if (this->player->checkIfAbility()) {
                    this->player->doAbility(sf::Vector2f(this->window->mapPixelToCoords(
                                                sf::Mouse::getPosition(*this->window), this->view)),
                                            this->projectileSystem, this->soundEngine);
        ...
        if (this->player->getSpawned()) {
            ...
            if (this->player->getName() != "warrior" && !this->playerGUI->getIsShopping()) {
                ...
                if (this->playerGUI->updateBuyingAbility(this->mousePosWindow,
                                                         this->getMouseClick(), this->soundEngine,
        ...
        if (this->waveCountdown < 10.f) {
            ...
            if (this->monsterSystem->monstersEmpty()) {
                ...
                this->player->attackMonster(this->monsterSystem, this->floatingTextSystem,
                                            this->soundEngine);
                this->monsterSystem->update(this->player, this->playerGUI, this->projectileSystem,
                                            this->dropSystem, this->floatingTextSystem,
                                            this->soundEngine, this->paused, dt);
                this->playerGUI->updateBossHP(dt);
            }
        }

        this->projectileSystem->update(this->player, this->playerGUI, this->particleSystem,
                                       this->monsterSystem, this->background, this->tileMap,
                                       this->floatingTextSystem, this->soundEngine, dt);

        this->dropSystem->update(this->player, this->playerGUI, this->floatingTextSystem,
                                 this->soundEngine, dt);
