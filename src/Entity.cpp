...
{
    return this->velocity != sf::Vector2f(0.f, 0.f);
}
...
{
    return sf::Vector2f(
        this->sprite.getPosition().x + (this->sprite.getGlobalBounds().width * 0.5f),
        this->sprite.getPosition().y + (this->sprite.getGlobalBounds().height * 0.5f));
}

const sf::Vector2f Entity::getLeftCenter() const
{
    return sf::Vector2f(
        this->sprite.getPosition().x + (this->sprite.getGlobalBounds().width * 0.125f),
        this->sprite.getPosition().y + (this->sprite.getGlobalBounds().height * 0.5f));
}
...
const sf::Vector2f Entity::getUpCenter() const
{
    return sf::Vector2f(
        this->sprite.getPosition().x + (this->sprite.getGlobalBounds().width * 0.5f),
        this->sprite.getPosition().y + (this->sprite.getGlobalBounds().height * 0.125f));
}
...
const sf::Vector2f Entity::getDownCenter() const
{
    return sf::Vector2f(
        this->sprite.getPosition().x + (this->sprite.getGlobalBounds().width * 0.5f),
        this->sprite.getPosition().y + (this->sprite.getGlobalBounds().height * 0.875f));
}

const sf::Vector2f Entity::getRightCenter() const
{
    return sf::Vector2f(
        this->sprite.getPosition().x + (this->sprite.getGlobalBounds().width * 0.875f),
        this->sprite.getPosition().y + (this->sprite.getGlobalBounds().height * 0.5f));
}
        ...
        if (vectorDistance(this->sprite.getPosition(), tileMap->getPosition(i)) < distance) {
            ...
            if (obstacleBounds.intersects(nextPos)) {
                if (bottomCollision(spriteBounds, obstacleBounds)) {
                    ...
                    this->velocity.y = 0.f;
                    this->sprite.setPosition(spriteBounds.left,
                                             obstacleBounds.top + obstacleBounds.height);
                }
...
void Entity::animation(const float &dt)
{
    ...
    if (this->isAttacking && (this->velocity.x != 0.f || this->velocity.y != 0.f)) {
        ...
        if (this->animationCooldown >= 1.f) {
            ...
            sf::IntRect intRect(this->frame, y * 16 * monsterTile, 16 * monsterTile,
                                16 * monsterTile);
