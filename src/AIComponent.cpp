...
AIComponent::AIComponent(TileMap *tileMap, sf::VideoMode &vm) : vm(vm)
{
    ...
    for (size_t i = 0; i < tileMap->getSize(); ++i) {
        Nodes[static_cast<size_t>(tileMap->getPosition(i).x / this->BlockSize.x)]
             [static_cast<size_t>(tileMap->getPosition(i).y / this->BlockSize.y)]
                 .isWall = true;
    }
}
...
{
    Start = &Nodes[int(e2->getPosition().x / this->BlockSize.x)]
                  [int(e2->getPosition().y / this->BlockSize.y)];

    End = &Nodes[int(e1->getPosition().x / this->BlockSize.x)]
                [int(e1->getPosition().y / this->BlockSize.y)];
...
void AIComponent::AStarAlg()
{
    ...
    auto GetDist = [](Node *P1, Node *P2) {
        return vectorDistance(float(P2->x), float(P1->x), float(P2->y), float(P2->y));
    };
    ...
    while (!NodesToTest.empty()) {
        std::sort(NodesToTest.begin(), NodesToTest.end(),
...
void AIComponent::update(Entity *e1, Entity *e2, const std::vector<sf::Vector2f> &positions,
                         const float &vel)
{
    if (!this->activateAI) {
    ...
    if (this->activateAI) {
        if (Current != nullptr) {
            ...
            t.setPosition(
                sf::Vector2f(Current->x * BlockSize.x + 24, Current->y * BlockSize.y + 24));
