...
Projectile::Projectile(const sf::VideoMode &vm, const std::string &projectile_name,
                       sf::Texture &texture, const float &x, const float &y, const uint32_t &attack,
                       const uint32_t &HP, const uint32_t &speed, const sf::Vector2f &coords,
...
void Projectile::calculateVelocity(const sf::Vector2f &coords)
{
    const float addAngle =
        getAngle(this->sprite.getPosition().x, this->sprite.getPosition().y, coords.x, coords.y) +
        90.f;
...
void Projectile::playerCollision(Player *player)
{
    if (this->name == "stone" || this->name == "groundWave") {
        ...
        if (vectorDistance(this->sprite.getPosition(), player->getPosition()) < distance &&
            ...
            if (playerBounds.intersects(nextPos)) {
                if (bottomCollision(projectileBounds, playerBounds)) {
                    this->velocity.y = 0.f;
                    this->sprite.setPosition(projectileBounds.left,
                                             playerBounds.top - projectileBounds.height);
                ...
                if (rightCollision(projectileBounds, playerBounds)) {
                    this->velocity.x = 0.f;
                    this->sprite.setPosition(playerBounds.left - projectileBounds.width,
                                             projectileBounds.top);
                    this->collidedPlayer = true;
                }
                                  ...
                                  FloatingTextSystem *floatingTextSystem)
{
    if (this->name == "shuriken" || this->name == "bomb") {
        ...
        if (vectorDistance(this->sprite.getPosition(), monster->getPosition()) < distance &&
            !this->collided && !this->collidedPlayer && !this->collidedMonster) {
            ...
            if (mobBounds.intersects(nextPos)) {
                if (bottomCollision(projectileBounds, mobBounds)) {
                    this->velocity.y = 0.f;
                    this->sprite.setPosition(projectileBounds.left,
                                             mobBounds.top - projectileBounds.height);
                ...
                if (rightCollision(projectileBounds, mobBounds)) {
                    this->velocity.x = 0.f;
                    this->sprite.setPosition(mobBounds.left - projectileBounds.width,
                                             projectileBounds.top);
                    this->collidedMonster = true;
                }
...
{
    const float vel =
        (this->speed * 0.2f + 0.8f) * 16.f * this->sprite.getGlobalBounds().width * dt;
