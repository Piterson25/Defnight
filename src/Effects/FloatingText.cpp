    ...
    : gui(isgui)
{
    ...
    this->text->setAlphaColor(static_cast<uint8_t>(0));
    ...
    if (this->moveCooldown < 1.5f) {
        ...
        if (this->moveCooldown < 0.5f) {
            this->text->setAlphaColor(
                static_cast<uint8_t>(this->moveCooldown * 510.f));
        }
        else if (this->moveCooldown >= 1.f) {
            this->text->setAlphaColor(
                static_cast<uint8_t>(255.f - (this->moveCooldown * 510.f)));
        }
        else {
            this->text->setAlphaColor(static_cast<uint8_t>(255));
        }
    }
}
