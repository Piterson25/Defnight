...
Monster::Monster(const sf::VideoMode &vm, const std::string &monster_name, sf::Texture &texture,
                 sf::Texture &shadow_texture, TileMap *tileMap, const float &x, const float &y,
                 const float &difficulty_mod, const float &wave_mod)
{
    this->vm = vm;
...
const bool Monster::attackPlayer(Player *player, TileMap *tileMap,
                                 ...
                                 FloatingTextSystem *floatingTextSystem, SoundEngine *soundEngine)
{
    ...
    if (this->name == "cyclope" && distance <= this->reach * 8.f * calcX(64, vm)) {
        if (!sightCollision(tileMap,
                            sf::Vector2f(this->getPosition().x + calcX(24, vm),
                                         this->getPosition().y + calcY(36, vm)),
                            player->getCenter())) {
            this->doAttack();
            if (!player->isDead() && !player->getPunched() && this->getIsAttacking() &&
                this->getFrame() == 80) {
                if (this->getAttack() > 0) {
                    projectileSystem->addProjectile(
                        "stone", this->getPosition().x + calcX(24, vm),
                        this->getPosition().y + calcY(36, vm), this->getAttack(), 1, 2,
        ...
        this->doAttack();
        if (!player->isDead() && !player->getPunched() && this->getIsAttacking() &&
            this->getFrame() == 80 * this->monsterSize) {
            int Lattack =
                static_cast<int>(round(this->attack - (this->attack * player->getArmor() * 0.05f)));

            if (Lattack > 0) {
                floatingTextSystem->addFloatingText(std::to_string(-Lattack), calcChar(16, vm),
...
const bool Monster::sightCollision(TileMap *tileMap, const sf::Vector2f &a_p1,
                                   const sf::Vector2f &a_p2)
{
    for (size_t i = 0; i < tileMap->getSize(); ++i) {
        if (vectorDistance(this->sprite.getPosition(), tileMap->getPosition(i)) <=
            20.f * calcX(32, vm)) {
...
void Monster::spawn(const float &dt)
{
    if (this->spawnCountdown < 0.5f) {
        this->sprite.setColor(
            sf::Color(255, 255, 255, static_cast<sf::Uint8>(this->spawnCountdown * 510.f)));
...
const bool Monster::dying(const float &dt)
{
    if (this->deadCountdown < 0.25f) {
        this->sprite.setColor(sf::Color(
            255, 255, 255, static_cast<sf::Uint8>(255.f - (this->deadCountdown * 1020.f))));
...
void Monster::calculateAI(TileMap *tileMap, Player *player,
                          const std::vector<sf::Vector2f> &positions, const float &dt)
{
