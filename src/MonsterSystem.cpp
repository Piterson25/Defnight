...
MonsterSystem::MonsterSystem(const sf::VideoMode &vm, TileMap *tileMap, const float &gridSize,
                             const float &difficulty_mod)
...
void MonsterSystem::addMonster(const std::string &monster_name, sf::Texture &texture,
                               const float &x, const float &y, const float &wave_mod)
{
    this->monsters.emplace_back(new Monster(this->vm, monster_name, texture, this->shadow_texture,
                                            this->tileMap, x, y, this->difficulty_mod, wave_mod));
}

void MonsterSystem::addBoss(const std::string &monster_name, sf::Texture &texture, const float &x,
                            const float &y, const float &wave_mod)
{
    this->monsters.emplace_back(new Boss(this->vm, monster_name, texture, this->shadow_texture,
                                         this->tileMap, x, y, this->difficulty_mod, wave_mod, 2));
}

void MonsterSystem::playerAttack(Player *player, FloatingTextSystem *floatingTextSystem,
                                 SoundEngine *soundEngine, bool &playedSound)
{
    for (const auto &monster : monsters) {
        ...
        if (distance <= player->getReach() * calcX(32, vm)) {
            ...
            if (!monster->isDead() && !monster->getPunched() && monster->getSpawned() &&
                player->getIsAttacking() && player->getFrame() == 80) {
                if ((static_cast<uint32_t>(Random::Float() * 100.f) + 1) <=
                    player->getCriticalChance()) {
                    const int attack = 2 * player->getAttack();
                    floatingTextSystem->addFloatingText(std::to_string(-attack), calcChar(16, vm),
...
void MonsterSystem::spawnMonsters(Player *player, const uint32_t &wave)
{
    ...
    for (const auto &id : this->monsterIDs) {
        ...
        for (size_t i = 0; i < this->tileMap->getSize(); ++i) {
            while ((this->tileMap->getPosition(i).x == calcX(this->gridSize * rx, this->vm) &&
                    this->tileMap->getPosition(i).y == calcY(this->gridSize * ry, vm)) ||
                   vectorDistance(calcX(this->gridSize * rx, this->vm),
                                  calcY(this->gridSize * ry, this->vm), player->getPosition().x,
            ...
            for (const auto &mob : this->monsters) {
                if (mob->getPosition().x == calcX(this->gridSize * rx, this->vm) &&
                    mob->getPosition().y == calcY(this->gridSize * ry, this->vm)) {
                    rx = static_cast<uint32_t>(Random::Float() * 30.f) + 1;
                    ry = static_cast<uint32_t>(Random::Float() * 30.f) + 1;
                    i = 0;
                }
        ...
        switch (id) {
            case 0:
                this->addMonster("goblin", this->textures["GOBLIN"],
                                 calcX(this->gridSize * rx, this->vm),
                                 calcY(this->gridSize * ry, this->vm), wave_mod);
                break;
            case 1:
                this->addMonster("spider", this->textures["SPIDER"],
                                 calcX(this->gridSize * rx, this->vm),
                                 calcY(this->gridSize * ry, this->vm), wave_mod);
                break;
            case 2:
                this->addMonster("orc", this->textures["ORC"], calcX(this->gridSize * rx, this->vm),
                                 calcY(this->gridSize * ry, this->vm), wave_mod);
                break;
            case 3:
                this->addMonster("cyclope", this->textures["CYCLOPE"],
                                 calcX(this->gridSize * rx, this->vm),
                                 calcY(this->gridSize * ry, this->vm), wave_mod);
                break;
            case 4:
                this->addBoss("minotaur", this->textures["MINOTAUR"],
                              calcX(this->gridSize * 16, this->vm),
                              calcY(this->gridSize * 16, this->vm), wave_mod);
                break;
            default:
                break;
        }
...
void MonsterSystem::update(Player *player, PlayerGUI *playerGUI, ProjectileSystem *projectileSystem,
                           DropSystem *dropSystem, FloatingTextSystem *floatingTextSystem,
                           SoundEngine *soundEngine, bool &paused, const float &dt)
{
    for (const auto &monster : this->monsters) {
        if (monster->getSpawned()) {
            if (monster->isDead()) {
                ...
                if (monster->attackPlayer(player, this->tileMap, projectileSystem,
    ...
    for (auto monster = this->monsters.begin(); monster != this->monsters.end();) {
        if ((*monster)->getDeadCountdown()) {
            ...
            playerGUI->update_XP();
            dropSystem->addDrop("coin", (*monster)->getPosition().x + calcX(16, vm),
                                (*monster)->getPosition().y + calcY(16, vm), (*monster)->getGold());
            if (const uint8_t t = uint8_t(Random::Float() * 4);
                (this->difficulty_mod == 0.75f && t < 2) || t == 0) {
                dropSystem->addDrop("heart", (*monster)->getPosition().x + calcX(16, vm),
