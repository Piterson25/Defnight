...
void SettingsState::initGUI()
{
    ...
    this->texts["SETTINGS"] =
        new gui::Text(&this->font, this->lang["SETTINGS"], calcChar(32, vm), calcX(640, vm),
    ...
    for (size_t i = 0; i < this->videoModes.size(); ++i) {
        if (this->videoModes[i] == vm) {
            ...
            this->text_buttons["RESOLUTION"] = new gui::ButtonText(
                &this->font,
                this->lang["RESOLUTION"] + std::to_string(this->videoModes[this->id].width) + "x" +
                    std::to_string(this->videoModes[this->id].height),
                calcChar(16, vm), calcX(96, vm), calcY(192, vm), sf::Color(255, 255, 255),
                sf::Color(192, 192, 192), false);
            break;
        }
    }
    ...
    if (this->gameSettings->fullscreen) {
        ...
        this->text_buttons["FULLSCREEN"] =
            new gui::ButtonText(&this->font, this->lang["FULLSCREEN"] + this->lang["YES"],
        ...
        this->text_buttons["FULLSCREEN"] =
            new gui::ButtonText(&this->font, this->lang["FULLSCREEN"] + this->lang["NO"],
                                calcChar(16, vm), calcX(96, vm), calcY(242, vm),
                                sf::Color(255, 255, 255), sf::Color(192, 192, 192), false);
    }
        ...
        if (this->fpsLimits[i] == this->gameSettings->fpsLimit) {
            ...
            if (this->fpsLimits[i] == 0) {
                this->text_buttons["FPS_LIMIT"] = new gui::ButtonText(
                    &this->font, this->lang["FPS_LIMIT"] + this->lang["NO_LIMIT"], calcChar(16, vm),
                    calcX(96, vm), calcY(292, vm), sf::Color(255, 255, 255),
                    sf::Color(192, 192, 192), false);
            }
            else {
                this->text_buttons["FPS_LIMIT"] = new gui::ButtonText(
                    &this->font,
                    this->lang["FPS_LIMIT"] + std::to_string(this->fpsLimits[this->fps_id]),
                    calcChar(16, vm), calcX(96, vm), calcY(292, vm), sf::Color(255, 255, 255),
                    sf::Color(192, 192, 192), false);
            }
            break;
        }
    ...
    for (size_t i = 0; i < this->musicVolumes.size(); ++i) {
        if (this->musicVolumes[i] == this->musicVolume) {
            ...
            this->text_buttons["MUSIC"] = new gui::ButtonText(
                &this->font,
                this->lang["MUSIC"] + std::to_string(this->musicVolumes[this->musicVolume_id]) +
                    "%",
                calcChar(16, vm), calcX(96, vm), calcY(342, vm), sf::Color(255, 255, 255),
                sf::Color(192, 192, 192), false);
            break;
        }
    }
    ...
    for (size_t i = 0; i < this->soundsVolumes.size(); ++i) {
        if (this->soundsVolumes[i] == this->soundsVolume) {
            ...
            this->text_buttons["SOUNDS"] = new gui::ButtonText(
                &this->font,
                this->lang["SOUNDS"] + std::to_string(this->soundsVolumes[this->soundsVolume_id]) +
                    "%",
                calcChar(16, vm), calcX(96, vm), calcY(392, vm), sf::Color(255, 255, 255),
                sf::Color(192, 192, 192), false);
            break;
        }
    }
    ...
    if (this->gameSettings->language == "english") {
        ...
        this->text_buttons["LANGUAGE"] =
            new gui::ButtonText(&this->font, this->lang["LANGUAGE"] + this->lang["ENGLISH"],
        ...
        this->text_buttons["LANGUAGE"] =
            new gui::ButtonText(&this->font, this->lang["LANGUAGE"] + this->lang["POLISH"],
                                calcChar(16, vm), calcX(96, vm), calcY(442, vm),
                                sf::Color(255, 255, 255), sf::Color(192, 192, 192), false);
    }
    ...
    if (this->gameSettings->fpsCounterOn) {
        ...
        this->text_buttons["FPS_COUNTER"] =
            new gui::ButtonText(&this->font, this->lang["FPS_COUNTER"] + this->lang["ON"],
        ...
        this->text_buttons["FPS_COUNTER"] =
            new gui::ButtonText(&this->font, this->lang["FPS_COUNTER"] + this->lang["OFF"],
                                calcChar(16, vm), calcX(96, vm), calcY(492, vm),
                                sf::Color(255, 255, 255), sf::Color(192, 192, 192), false);
    }

    this->texts["MOVEMENT"] =
        new gui::Text(&this->font, this->lang["MOVEMENT"], calcChar(16, vm), calcX(800, vm),
                      calcY(192, vm), sf::Color(255, 255, 255), false);
    this->texts["LEFT_CLICK"] =
        new gui::Text(&this->font, this->lang["LEFT_CLICK"], calcChar(16, vm), calcX(800, vm),
                      calcY(242, vm), sf::Color(255, 255, 255), false);
    this->texts["RIGHT_CLICK"] =
        new gui::Text(&this->font, this->lang["RIGHT_CLICK"], calcChar(16, vm), calcX(800, vm),
                      calcY(292, vm), sf::Color(255, 255, 255), false);
    this->texts["ESCAPE"] =
        new gui::Text(&this->font, this->lang["ESCAPE"], calcChar(16, vm), calcX(800, vm),
                      calcY(342, vm), sf::Color(255, 255, 255), false);
    this->texts["SHOP"] =
        new gui::Text(&this->font, this->lang["SHOP"], calcChar(16, vm), calcX(800, vm),
                      calcY(392, vm), sf::Color(255, 255, 255), false);
    this->texts["RUN"] =
        new gui::Text(&this->font, this->lang["RUN"], calcChar(16, vm), calcX(800, vm),
                      calcY(442, vm), sf::Color(255, 255, 255), false);
    this->texts["BUY_ABILITY"] =
        new gui::Text(&this->font, this->lang["BUY_ABILITY"], calcChar(16, vm), calcX(800, vm),
                      calcY(492, vm), sf::Color(255, 255, 255), false);

    this->text_buttons["APPLY"] = new gui::ButtonText(
        &this->font, this->lang["APPLY"], calcChar(32, vm), calcX(1000, vm), calcY(592, vm),
        sf::Color(255, 255, 255), sf::Color(192, 192, 192), false);
    this->text_buttons["APPLY"]->setPosition(
        sf::Vector2f(calcX(1184, vm) - this->text_buttons["APPLY"]->getWidth(), calcY(592, vm)));
}
...
void SettingsState::update(const float &dt)
{
    ...
    if (this->text_buttons["FPS_LIMIT"]->isPressed() && !this->getMouseClick()) {
        ...
        if (this->fpsLimits[fps_id] == 0) {
            this->text_buttons["FPS_LIMIT"]->setText(this->lang["FPS_LIMIT"] +
                                                     this->lang["NO_LIMIT"]);
        }
        else {
            this->text_buttons["FPS_LIMIT"]->setText(this->lang["FPS_LIMIT"] +
    ...
    if (this->text_buttons["FPS_COUNTER"]->isPressed() && !this->getMouseClick()) {
        ...
        if (this->fpsCounterOn) {
            this->fpsCounterOn = false;
            this->text_buttons["FPS_COUNTER"]->setText(this->lang["FPS_COUNTER"] +
                                                       this->lang["OFF"]);
        }
        else {
            this->fpsCounterOn = true;
