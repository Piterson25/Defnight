...
Drop::Drop(const std::string &name, const float &posX, const float &posY, const uint32_t &worth,
           const bool &vanishing, sf::VideoMode &vm)
...
void Drop::spawn(const float &dt)
{
    if (this->spawnCountdown < 0.25f) {
        this->sprite.setColor(
            sf::Color(255, 255, 255, static_cast<sf::Uint8>(this->spawnCountdown * 1020.f)));
...
void Drop::move(const float &posX, const float &posY, const float &dt)
{
    this->angle = getAngle(this->sprite.getPosition().x + calcX(8, vm),
                           this->sprite.getPosition().y + calcY(8, vm), posX + calcX(32, vm),
                           posY + calcY(32, vm)) +
                  90.f;

    this->velocity.x =
        this->sprite.getGlobalBounds().width * 16.f * cos((3.1415f / 180) * this->angle) * dt;
    this->velocity.y =
        this->sprite.getGlobalBounds().width * 16.f * sin((3.1415f / 180) * this->angle) * dt;
...
const bool Drop::playerPick(Player *player, PlayerGUI *playerGUI,
                            ...
                            const float &dt)
{
    const float distance = vectorDistance(sf::Vector2f(this->sprite.getPosition().x + calcX(8, vm),
    ...
    if (distance <= player->getReach() * calcX(16, vm)) {
        if (this->name == "coin") {
            ...
            floatingTextSystem->addFloatingText("+" + std::to_string(this->worth), calcChar(16, vm),
